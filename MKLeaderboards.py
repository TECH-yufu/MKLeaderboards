...
class MKLeaderboard():
    def __init__(self, category, track='none'):
        ...
        if self.category == 'no-sc' or self.category == 'unrestricted':
            ...
            # self.urls = self.get_YouTubeURLs(self.mkwii_data, self.track_names)
    ...
    # def get_YouTubeURLs(self, mkwii_data, track_names):
    #     URLs = {}
    #
    #     for track,i in zip(track_names, np.arange(0,32)):
    #         URLs[track] = [mkwii_data[i]['data'][j]['proof'] for j in np.arange(0,10)]
    #
    #     return URLs
    ...
    def WW_tops_by_player(self,category, player_names):
        ...
        plt.savefig('playertop.png', bbox_inches='tight')
    ...
    def WW_tops_by_country(self,category, country_names):
        ...
        plt.savefig('countrytop.png',bbox_inches='tight')
        plt.clf() # clear figure
    


if __name__ == '__main__':
    wii = MKLeaderboard(category='no-sc', track='grumble volcano')
